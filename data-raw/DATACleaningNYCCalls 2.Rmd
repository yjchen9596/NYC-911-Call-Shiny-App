...
```{r}
data <-read_rds("~/Desktop/DATA613/groupproject_backup/NYPD_Calls_ALL.rds")
```
...
calls <- data |> 
  ...
  mutate(category = str_extract(case_types, "([:alpha:]*)\\:")) |> 
  select(-cad_evnt_id, -create_date, -nypd_pct_cd, -geo_cd_x, -geo_cd_y, -add_ts, -disp_ts, -closng_ts, -radio_code)

#write_rds(calls, file = "NYPD_Calls_ALL_Clean.rds", compress = "gz")
