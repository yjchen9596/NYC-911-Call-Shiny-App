...
density$gap<-as.double(substr(density$gap,1,4))
...
ggplot(lg, aes(x=gap, fill = boro_nm))+
...
```{r}
ggplot(ss, aes(x="", y=summ, fill=boro_nm))+
  geom_col() +
  coord_polar("y")+
  theme_void()
```
...
lk|>
  ...
  summarize(cnt = sum(incident_month-incident_month+1))->lk2
